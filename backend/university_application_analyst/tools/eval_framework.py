...
class EnglishProficiencyAnalysis(BaseModel):
    ...
    minimum_required: float = Field(description="Minimum score required (TOEFL 79, IELTS 6.5)")
    meets_minimum: bool = Field(description="Whether score meets threshold")
    margin: float = Field(description="Points above/below minimum")
...
class GREAnalysis(BaseModel):
    ...
    quantitative_score: float = Field(ge=130, le=170, description="GRE Quantitative score")
    quantitative_percentile: float = Field(ge=0, le=100, description="Quantitative percentile")
    verbal_score: float = Field(ge=130, le=170, description="GRE Verbal score")
    verbal_percentile: float = Field(ge=0, le=100, description="Verbal percentile")
    awa_score: float = Field(ge=0, le=6, description="Analytical Writing Assessment score")
    awa_percentile: float = Field(ge=0, le=100, description="AWA percentile")
    composite_percentile: float = Field(description="Weighted composite percentile (Quant 70%, Verbal 20%, AWA 10%)")
